...
const Getters = {
  ...
  liveApps: (state, getters) => {
    let apps = state.apps.filter(a => a.available);

    const appList = getters.option('apps') || [];
    apps = apps.filter(a => (a.required || a.enabled) && _.includes(appList, a.id) && a.live);
  ...
  // Return a value (or undefined) for a given option path.
  ...
  option: state => key => _.at(state.options.combined, key)[0],
