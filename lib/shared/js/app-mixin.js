 ...
 */
export default {
  props: ['id', 'enabled', 'shown', 'clicked'],
  ...
  created() {
    if (typeof this.id === 'undefined') {
      throw new Error('App mixin cannot be used without a valid id prop');
    }

    this.muxy = new Muxy.SDK(this.id);

    // Listen for app config changes and set them globally.
    this.muxy.listen(Events.APP_OPTIONS_CHANGE, (options) => {
  ...
  watch: {
    ...
    /**
     * Watch the `clicked` property and emit a clicked event when it changes.
     */
    clicked() {
      this.$emit('clicked');
    }
  ...
  methods: {
    ...
    option(path, def) {
      ...
      let cb = typeof def === 'function' ? def : null;
    ...
    saveChannelOptions(options) {
      ...
      const promise = this.muxy.setChannelState(options).then(() => {
        this.muxy.send(Events.APP_OPTIONS_CHANGE, this.option());
      });
